  ...
  try {
    ...
    for (const entry of initialComponents) {
      if (!components.has(entry.key)) {
        ...
        const dbCfg = await (prisma.componentConfig as any).create({
          data: {
            configKey: entry.key,
...
const server = http.createServer(async (req: http.IncomingMessage, res: http.ServerResponse) => {
  ...
  if (req.method === "GET" && url.pathname === "/admin/stats") {
    ...
    try {
      ...
      const toolDistribution = distribution.reduce((acc: any, curr) => {
  ...
  if (req.method === "POST" && url.pathname === "/components") {
    ...
    const variants = (rawConfig as any).variants || [];
    const figmaKey = (rawConfig as any).figma?.componentKey || "";
